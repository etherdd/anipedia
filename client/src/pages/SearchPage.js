...
export default function SearchPage() {
  ...
  const searchPerson = () => {
    fetch(
      ...
      .then((res) => res.json())
      .then((resJson) => {
        const personWithId = resJson.map((person) => ({
          id: `${person.name_id}_${person.category}_${person.imdb_id}`,
          ...person,
        }));
  ...
  const columns = [
    {
      ...
      renderCell: (params) => (
        <NavLink
          style={{ color: "white" }}
          to={`/movie-info/${params.row.imdb_id}`}
        >
          {params.value}
        </NavLink>
      ),
  ...
  if (searchType === SEARCH_PERSON) {
    ...
    const nameColumn = {
      ...
      renderCell: (params) => (
        <NavLink
          style={{ color: "white" }}
          to={`/person-info/${params.row.name_id}`}
        >
          {params.value}
        </NavLink>
      ),
            ...
            <>
              ...
              <Grid item xs={1.5}>
                <TextField
                  ...
                  onChange={(e) =>
                    setMaxRuntimePerson(Math.max(0, +e.target.value))
                  }
                  inputProps={{ min: "0" }}
                  style={{ width: "100%" }}
                />
