...
export default function MovieInfoPage() {
  ...
  useEffect(() => {
    fetch(`${process.env.NODE_ENV === 'production' ? config.production_server_host : config.server_host}:${config.server_port}/movie/${movie_id}`)
      .then(res => res.json())
  ...
  const handleLikeClick = () => {
    ...
    fetch(
      `${process.env.NODE_ENV === 'production' ? config.production_server_host : config.server_host}:${config.server_port}/movie/${movie_id}/like`,
      {
        method: "POST",
        headers: {
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.NODE_ENV === 'production' ? config.production_server_host : config.server_host}:${config.server_port}/movie/${movie_id}/like?user_id=${user.sub}`)
      .then(res => res.json())
      .then(resJson => {
        console.log(resJson);
